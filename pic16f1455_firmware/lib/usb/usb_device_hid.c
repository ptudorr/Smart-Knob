...
extern const struct{uint8_t report[HAPTIC_RPT_SIZE];}HAPTIC_rpt;
extern const struct{uint8_t report[LED_RPT_SIZE];}LED_rpt;
extern const struct{uint8_t report[DISPLAY_RPT_SIZE];}DISPLAY_rpt;
...
void USBCheckHIDRequest(void)
{
    ...
    if(SetupPkt.bIntfID != HID_INTF_HAP_ID && SetupPkt.bIntfID != HID_INTF_LED_ID && SetupPkt.bIntfID != HID_INTF_DSP_ID ) return;
    ...
    if(SetupPkt.bRequest == USB_REQUEST_GET_DESCRIPTOR)
    {
        switch(SetupPkt.bDescriptorType)
        {
            case DSC_HID: //HID Descriptor
                ...
                {
                    if(SetupPkt.wIndex == HID_INTF_HAP_ID)
                        USBEP0SendROMPtr(
                    ...
                    if(SetupPkt.wIndex == HID_INTF_LED_ID)
                        USBEP0SendROMPtr(
                            (const uint8_t*)&configDescriptor1 + 50,		//18 is a magic number.  It is the offset from start of the configuration descriptor to the start of the HID descriptor.
                            sizeof(USB_HID_DSC)+3,
                            USB_EP0_INCLUDE_ZERO);
                    if(SetupPkt.wIndex == HID_INTF_DSP_ID)
                        USBEP0SendROMPtr(
                            (const uint8_t*)&configDescriptor1 + 82,		//18 is a magic number.  It is the offset from start of the configuration descriptor to the start of the HID descriptor.
                            sizeof(USB_HID_DSC)+3,
                            USB_EP0_INCLUDE_ZERO);
                }
                break;
            case DSC_RPT:  //Report Descriptor
                if(USBActiveConfiguration == 1)
                {
                    if(SetupPkt.wIndex == HID_INTF_HAP_ID)
                        USBEP0SendROMPtr(
                            (const uint8_t*)&HAPTIC_rpt,
                            HAPTIC_RPT_SIZE,     //See usbcfg.h
                            USB_EP0_INCLUDE_ZERO);
                    
                    if(SetupPkt.wIndex == HID_INTF_LED_ID)
                        USBEP0SendROMPtr(
                            (const uint8_t*)&LED_rpt,
                            LED_RPT_SIZE,     //See usbcfg.h
                            USB_EP0_INCLUDE_ZERO);
                    
                    if(SetupPkt.wIndex == HID_INTF_DSP_ID)
                        USBEP0SendROMPtr(
                            (const uint8_t*)&DISPLAY_rpt,
                            LED_RPT_SIZE,     //See usbcfg.h
                            USB_EP0_INCLUDE_ZERO);
                }
                break;
