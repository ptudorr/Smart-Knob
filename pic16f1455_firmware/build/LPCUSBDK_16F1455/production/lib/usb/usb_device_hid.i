...
# 2 "<built-in>" 2
# 1 "lib/usb/usb_device_hid.c" 2
# 44 "lib/usb/usb_device_hid.c"
...
# 29 ".\\include/usb/usb_hal_pic16f1.h"
...
# 29 ".\\include/usb/usb_hal_pic16f1.h" 2
...
# 46 "lib/usb/usb_device_hid.c" 2
# 67 "lib/usb/usb_device_hid.c"
typedef struct
{
    unsigned :8;
    unsigned :8;
...
extern const struct{uint8_t report[87];}HAPTIC_rpt;
extern const struct{uint8_t report[28];}LED_rpt;
extern const struct{uint8_t report[22];}DISPLAY_rpt;
# 163 "lib/usb/usb_device_hid.c"
void USBCheckHIDRequest(void)
{
    if(SetupPkt.Recipient != (0x01)) return;
    if(SetupPkt.bIntfID != 0x00 && SetupPkt.bIntfID != 0x01 && SetupPkt.bIntfID != 0x02 ) return;
        ...
        switch(SetupPkt.bDescriptorType)
        {
            case 0x21:
                if(USBActiveConfiguration == 1)
                {
                    ...
                    if(SetupPkt.wIndex == 0x01)
                        { inPipes[0].pSrc.bRom = (const uint8_t*)&configDescriptor1 + 50; inPipes[0].wCount.Val = sizeof(USB_HID_DSC)+3; inPipes[0].info.Val = 0x40 | 0x80 | 0x00; };



                    if(SetupPkt.wIndex == 0x02)
                        { inPipes[0].pSrc.bRom = (const uint8_t*)&configDescriptor1 + 82; inPipes[0].wCount.Val = sizeof(USB_HID_DSC)+3; inPipes[0].info.Val = 0x40 | 0x80 | 0x00; };
            ...
            case 0x22:
                ...
                {
                    if(SetupPkt.wIndex == 0x00)
                        { inPipes[0].pSrc.bRom = (const uint8_t*)&HAPTIC_rpt; inPipes[0].wCount.Val = 87; inPipes[0].info.Val = 0x40 | 0x80 | 0x00; };




                    if(SetupPkt.wIndex == 0x01)
                        { inPipes[0].pSrc.bRom = (const uint8_t*)&LED_rpt; inPipes[0].wCount.Val = 28; inPipes[0].info.Val = 0x40 | 0x80 | 0x00; };




                    if(SetupPkt.wIndex == 0x02)
                        { inPipes[0].pSrc.bRom = (const uint8_t*)&DISPLAY_rpt; inPipes[0].wCount.Val = 28; inPipes[0].info.Val = 0x40 | 0x80 | 0x00; };
